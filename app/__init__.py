...
class RequestFormatter(logging.Formatter):
    def format(self, record):
        if has_request_context():
            ...
            record.ip = request.headers.get('X-Forward-For', request.remote_addr)
...
def create_app():
    ...
    def log_request(response):
        ...
        f1 = (request.path == '/favicon.ico') or (request.path.startswith('/static')) or (request.path.startswith('/bootstrap'))
        if f1:
            return response

        # Cleaned up this part to get rid of the ugly elif statements.
